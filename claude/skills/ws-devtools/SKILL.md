...
plain host or devcontainer. Merged from long debugging sessions; current for
ws-devtools ≥ 0.1.2. Generic usage reference:
...
| `degraded: true` / tool errors "DEGRADED … pid \<n\>" | lost the :9999 race | §3 — `kill <n>` |
...
- The MCP server self-starts the bridge on `:9999` (hard-coded in the extension).
  ...
  container.
- **Critical invariant:** the `mcp.mjs` your session talks to over stdio must be the
  same process that owns :9999 — a split reports loudly as DEGRADED (§3).
...
## §3. DEGRADED mode → split server on your side

A different `mcp.mjs` owns :9999. **Do NOT ask the user to re-arm — that's not the
...
3. Auto-reconnects when the bridge (re)appears and survives navigation — but
   **reconnect after a bridge restart is not instant** (backoff, allow ~30 s) and
   sometimes needs a fresh click. `clients: 2–3` in `status` is normal (extension +
   page transports).
4. **Arm click not landing although the whole chain checks out** (bridge listening,
   ...
   worker is asleep or gave up retrying. Fix: **refresh the page (F5), then click
   the extension icon** — that combination reliably re-attaches.

## Quick diagnosis flowchart

```
status → degraded / "owned by pid"?  ──yes──►  §3 (kill pid; next tool call respawns)
      │no
      ▼
...
- **Navigation drops a frame**: after `navigate`, the next call may time out while
  the tab re-injects — wait a beat, retry `url` until it acks. Normal, not a failure.
...
- **`eval` takes an expression, not statements** — wrap in an IIFE:
  `(() => { foo(); return 'done'; })()`.
- **`waitfor` occasionally returns `Unknown command`** after a reconnect (client-path
  dependent) — fall back to polling via `eval`.
...
- **`screencast_start` → act → `screencast_frame settle:<ms>`** — the best act→verify
  ...
  sleeps, `unchanged:true` costs nothing. **Gotcha:** the stream can go stale after
  cross-SITE navigation — if a frame returns `unchanged` when the page clearly changed,
  fall back to `screenshot` (always current) or restart the screencast.
