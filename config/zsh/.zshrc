...
# libpython3.12 in ldconfig and lands on qsvpy312. Note the interpreter is the linked one,
# NOT whatever `python3` on PATH resolves to (mise ships 3.14 here) — `qsv py` sees 3.12's
# site-packages, or an active venv of the matching version.
#
...
#
# WHEN THE HOST HAS NO MATCHING LIBPYTHON: qsv v22.0.1 ships qsvpy311/312/313 only, and
# Ubuntu 26.04 (resolute) carries python3.14 and nothing else — neither main nor universe
# has a libpython3.11/3.12/3.13 — so every candidate aborted and the prune deleted all
# three. That is not recoverable with apt, and it failed silently while erasing its own
# evidence: afterwards the plugin dir is indistinguishable from a musl host that never
# shipped a qsvpy at all. So _qsv_fetch_python installs the newest CPython qsv actually
# ships a build for, and qsvpy becomes a wrapper rather than a symlink.
#
# WHY mise supplies it: its python-build-standalone tarballs already carry
# lib/libpython3.N.so.1.0 (the distro's -stdlib package does not), and mise is a hard
# dependency here anyway. The interpreter is deliberately NOT pinned in
# config/mise/config.toml — that would install a second python on every machine, including
# the ones whose apt libpython already works, and shift which python3 the shims resolve.
# The consequence is that `mise prune` sees a version no tracked config references and
# reclaims it; maintain::mise_prune protects it by exact tool@version instead. That is also
# why this must never become a `mise use`.
#
# WHAT the wrapper has to set, verified against qsvpy312 + a python-build-standalone 3.12:
# LD_LIBRARY_PATH alone is enough to link AND to import the stdlib — sys.prefix resolves to
# the mise install. PYTHONHOME only silences "Could not find platform dependent libraries
# <exec_prefix>" on stderr. macOS needs DYLD_LIBRARY_PATH and is UNVERIFIED here (SIP
# strips DYLD_* for hardened-runtime binaries), which is why the wrapper is smoke-tested
# after it is written and removed again if it cannot start.

# Ensures a CPython ${1} (e.g. 3.13) exists and prints its prefix. Idempotent: an existing
# install short-circuits before any network call, so atpull does not re-download on every
# plugin update. The prefix is baked into the wrapper, so a later patch bump would strand
# it — harmless, because atpull rewrites the wrapper and the smoke test below catches it.
function _qsv_fetch_python() {
    local ver="${1}" prefix
    (( $+commands[mise] )) || { print -u2 -- "qsv: mise not found; cannot supply libpython${ver}"; return 1; }
    prefix="$(mise where "python@${ver}" 2>/dev/null)"
    [[ -d "${prefix}" ]] && { print -r -- "${prefix}"; return 0; }
    print -u2 -- "qsv: no system libpython${ver}; installing python@${ver} via mise…"
    mise install "python@${ver}" >&2 || return 1
    prefix="$(mise where "python@${ver}" 2>/dev/null)"
    [[ -d "${prefix}" ]] || return 1
    print -r -- "${prefix}"
}

# Writes the qsvpy launcher: ${1} is the real binary, ${2} the interpreter prefix.
function _qsv_write_wrapper() {
    local bin="${1}" prefix="${2}" var='LD_LIBRARY_PATH'
    [[ "${OSTYPE}" == darwin* ]] && var='DYLD_LIBRARY_PATH'
    {
        print -r -- '#!/bin/sh'
        print -r -- '# Generated by _qsv_prune (config/zsh/.zshrc) — every zi update rewrites it.'
        print -r -- "PYTHONHOME='${prefix}'; export PYTHONHOME"
        print -r -- "${var}='${prefix}/lib'\${${var}:+:\$${var}}; export ${var}"
        print -r -- "exec '${PWD}/${bin}' \"\$@\""
    } > qsvpy
    chmod +x -- qsvpy
}
...
function _qsv_prune() {
    local f py prefix
    local -a keep=(qsv qsvmcp) cands
    rm -rf ._backup
    # Newest first; the first that starts is the one whose libpython this host resolves.
    cands=(qsvpy3<->(NOn))
    for f in "${cands[@]}"; do
        chmod +x -- "${f}"
        ./"${f}" --version >/dev/null 2>&1 && { py="${f}"; break; }
    done
    # Nothing on this host links: keep the newest build and bring it an interpreter.
    if [[ -z "${py}" && -n "${cands[1]}" ]]; then
        prefix="$(_qsv_fetch_python "3.${cands[1]#qsvpy3}")" && py="${cands[1]}"
    fi
    [[ -n "${py}" ]] && keep+=("${py}")
    # Runs before the relink, so the previous version's qsvpy is cleared first.
    for f in qsv[a-z]*(N); do
        (( ${keep[(Ie)${f}]} )) || rm -rf -- "${f}"
    done
    if [[ -n "${py}" && -n "${prefix}" ]]; then
        _qsv_write_wrapper "${py}" "${prefix}"
        ./qsvpy --version >/dev/null 2>&1 || {
            print -u2 -- "qsv: ${py} still cannot load libpython from ${prefix}; dropping the wrapper"
            rm -f -- qsvpy
        }
    elif [[ -n "${py}" ]]; then
        ln -sfn -- "${py}" qsvpy
    else
        (( ${#cands} )) && print -u2 -- "qsv: no usable interpreter for ${cands[*]}; 'qsv py' unavailable"
    fi
    return 0
}
...
for _fn_file in "${ZDOTDIR}/functions/"*.zsh(N); do
	source "${_fn_file}"
done
unset _fn_file
