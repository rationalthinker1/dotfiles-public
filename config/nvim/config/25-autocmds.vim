" ~/.config/nvim/config/25-autocmds.vim — every autocommand, inside a named group.
"
...
" Briefly highlight the region just yanked. The Vim config gets this from the
" bundled `hlyank` package; Neovim has no such package but exposes the same
" behaviour as a Lua function, so it is wired up by hand here.
augroup nvim_hlyank
  autocmd!
  autocmd TextYankPost * silent! lua vim.hl.on_yank({ timeout = 200 })
augroup END
...
augroup vimrc_reload
  ...
  autocmd BufWritePost $MYVIMRC,~/.config/nvim/config/*.vim
