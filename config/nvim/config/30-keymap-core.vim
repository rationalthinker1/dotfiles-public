" ~/.config/nvim/config/30-keymap-core.vim — Vim-native keys and the helpers behind them.
"
" WHAT LIVES WHERE
"
...
" directory in 'runtimepath' — here ~/.local/share/nvim/site/spell/, gitignored so machine-specific
...
function! CloseBuffer() abort
  " Closes the buffer the way a browser closes a tab: keep the window layout,
  " and only quit when the last real buffer goes.
  "
  " Simpler than the Vim version, which has three NERDTree-aware branches.
  " snacks.explorer is a picker window rather than a persistent split, so there
  " is no file-tree window to keep alive or re-focus.
  if &buftype ==? 'quickfix'
    cclose
    return
  endif
  ...
  if l:total_buffers <= 1 && winnr('$') == 1
    quit!
  elseif l:total_buffers > 1 && winnr('$') == 1
    bprevious | bdelete! #
  else
    bdelete
  endif
