...
Plug 'tpope/vim-endwise'                  " Autoclose for zsh, python, ruby 
...
let g:python3_host_prog = expand('/opt/anaconda3/bin/python3.7')
...
command! -nargs=0 -bar SaveCs if &modified
			\|    if empty(bufname('%'))
				...
				\|    else
					...
					\|    endif
					\|endif
nnoremap <silent> <C-s> :<C-u>SaveCs<CR>
inoremap <C-s> <Esc>:SaveCs<CR>
...
set undodir=stdpath('data')/tmp/undo     " undo files
set backupdir=stdpath('data')/tmp/backup " backups
set directory=stdpath('data')/tmp/swap   " swap files
