" ~/.vim/config/20-options.vim — every 'set' in one place, assigned once.
"
...
set encoding=utf-8          " encoding of buffers, registers and viminfo
set termencoding=utf-8      " what Vim writes to the terminal; matches kitty
...
set undofile                " persist undo history across sessions (&undodir)
set undolevels=1000         " undo steps kept per buffer
set undoreload=10000        " max lines to preserve undo for across a reload
set backup                  " keep persistent backups (in &backupdir, see 00-xdg.vim)
set nowritebackup           " but skip the transient write-time copy: coc.nvim and
                            " some language servers misread the create/delete churn
set noswapfile              " no .swp; undofile + backup already cover recovery,
                            " and swap files fight the symlinked dotfiles tree
...
" Never offer these to :e / :find, and keep them out of NERDTree (which honours
" 'wildignore' via g:NERDTreeRespectWildIgnore in 05-plugin-vars.vim).
...
set updatetime=300          " coc uses this for CursorHold; the 4000ms default feels broken
...
set t_vb=                   " and empty the visual-bell terminal code
...
"--- Clipboard -------------------------------------------------------------
" Backed by the provider registered in 01-clipboard.vim.
set clipboard=unnamedplus   " yank and put go through the system clipboard (+)

" Fix background-colour tearing in kitty.
