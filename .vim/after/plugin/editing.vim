...
"--- lexima: auto-close brackets and quotes --------------------------------
" g:lexima_enable_basic_rules is 0, so the rules are installed explicitly here
" and the <CR> hook is then registered against a known rule set.
" Guarded on the plugin's own load marker, not exists('*lexima#...'): an autoload
" function does not exist until its file has been sourced, so that test is false
" at this point even when the plugin is installed.
if exists('g:loaded_lexima')
  call lexima#set_default_rules()
  " lexima's own <CR> handling is disabled; after/plugin/zz-completion.vim owns
  " <CR> so it can arbitrate between coc's popup and lexima's expansion.
